...
export default function HeroFallback2D() {
  ...
  useEffect(() => {
    ...
    const fetchStates = async () => {
      try {
        const res = await axios.get('http://localhost:8000/api/destinations/states', { timeout: 3000 });
    ...
    const fetchAlternative = async () => {
      try {
        const res = await axios.get('http://localhost:8000/api/anti-overtourism/alternatives?popular_destination=Manali', { timeout: 3000 });
        if (res.data && res.data.circuits && res.data.circuits.length > 0) {
          setFeaturedAlternative(res.data.circuits[0]);
        }
      } catch (err) {
